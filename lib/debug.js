	...
	reload() {
		...
		if(this.mod.settings.debug.skills) {
			...
			for(let options of [
				...
				['C_START_COMBO_INSTANT_SKILL', 5, _ => `[${_.targets.map(t => `${t.arrowId} ${gameId(t.gameId)} ${t.hitCylinderId}`).join(',')}]`],
				['C_START_INSTANCE_SKILL', 6, _ => [_.unk, `[${_.targets.map(t => `${t.arrowId} ${gameId(t.gameId)} ${t.hitCylinderId}`).join(',')}]`]],
