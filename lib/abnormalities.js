...
// Safe comparison helper — handles mixed BigInt/Number gameIds
function sameId(a, b) {
	return BigInt(a) === BigInt(b)
}

const INFINITE_DURATION = 0x7fffffff // plain Number for use after conversion
...
class SPAbnormals {
	constructor(mod) {
		...
		mod.hook('S_CREATURE_LIFE', 3, event => {
			if (sameId(event.gameId, this.player.gameId) && !event.alive) this.removeAll()
		})

		let abnormalityUpdate = (type, event) => {
			if (sameId(event.target, this.player.gameId)) {
				...
				// event.duration is BigInt from the packet; convert to Number for all math
				let duration = Number(event.duration)

				if (duration <= INFINITE_DURATION && !this.listWithShit[event.id]) {
					duration = Math.round(Math.max(duration - this.ping.min, 0))
					event.duration = duration // keep event consistent if forwarded
				}
	...
	_add(id, duration) {
		// duration is always a plain Number here
		this.mod.clearTimeout(this.myAbnormals[id])
		this.myAbnormals[id] = duration >= INFINITE_DURATION ? true : this.mod.setTimeout(() => { this.remove(id) }, duration)
	}
